...
//            System.out.println("readToken detected eof");
    ...
    private boolean didReadStringLiteral() {
        ...
        if (currentChar == '"') {
            ...
            currentSpelling.append(currentChar); // add the opening double quote (") to our spelling
            ...
            while (inString) {
                ...
                if (currentChar == '"') {
                    currentSpelling.append(currentChar); // add the closing double quote (") to our spelling
                    accept(); // Let's accept the closing double quote (")
                    inString = false;
                    
                // have we detected an unterminated string?
                } else if (currentChar == '\n' || currentChar == '\r' || currentChar == sourceFile.eof) { 
                    errorReporter.reportError(currentSpelling + " : unterminated string", "Token.STRINGLITERAL", sourcePos);
                    accept();
                    inString = false;
                ...
                // have we detected an escape character?
                    ...
                    if (isLegalEscapeCharacter(currentChar)) {
                        ...
                        errorReporter.reportError("\\" + currentChar + " : illegal escape character", "Token.STRINGLITERAL", sourcePos);
